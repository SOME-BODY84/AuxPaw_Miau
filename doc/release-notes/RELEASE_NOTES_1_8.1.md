...
NYAN Core 1.8.1 is primarily a bugfix release, bringing NYAN Core in
line with Bitcoin 0.9.3. NYAN Core 1.8.1 also adds in support for printing
...
* Disable Transifex based translation process (not used by NYAN Core)
* Add checkpoints around AuxPoW switchover block (371,337)
* Disable connections from pre-1.8 NYAN Core clients
* Use HTTPS with nyanchain.info
...
There is a known issue with quantities in JSON used by the RPC interface (https://github.com/nyan/nyan/issues/134).
...
This only affects values significantly below 1 NYAN, however merchants may wish to round to
the nearest NYAN on all transactions for now.
